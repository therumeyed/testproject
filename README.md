...
`POST /admin/reset-mentions` (with header `Authorization: Bearer <ADMIN_TOKEN>`) truncates `mentions` and `ingest_runs` — useful while still tuning the pipeline (query phrasing, source list, etc.) and old data no longer matches the current setup. `ADMIN_TOKEN` is auto-generated by Render (`generateValue: true` in `render.yaml`) — find it under the web service's Environment tab, or set your own for local testing.

This exists as an HTTP endpoint rather than "just run a SQL command" because Render's Postgres is only reachable via direct TCP connection from inside Render's network or a machine with `psql`/a DB GUI — there's no browser-based SQL console for it. Going through the already-deployed web service (which does have DB access) over plain HTTPS sidesteps needing either.

```bash
curl -X POST -H "Authorization: Bearer <ADMIN_TOKEN>" https://<your-web-service>.onrender.com/admin/reset-mentions
```
